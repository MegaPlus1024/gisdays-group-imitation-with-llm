...
.\.venv\Scripts\python.exe scripts/run_single_trial_controlled.py `
  ...
  --local-pipeline-config artifacts/first_run_packets/phase_8_26_mini_matrix_r3/local_pipeline_config.r03.json `
  --output-dir artifacts/single_trial_runs/phase_8_26_mini_matrix_r3 `
  --trial-id office_document_file_workflow_basic_v1__second_model__to__first_model__r03 `
  --allow-runtime-execution `
  --confirm-runtime-execution SINGLE_TRIAL_RUNTIME_OPT_IN `
  --auto-matrix-adapter-outputs `
  --run-id phase_8_26_mini_matrix_r3 `
