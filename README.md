...
- Phase 13E adds a read-only stateful workflow foundation in `docs/status/phase_13e_readonly_stateful_workflows.md`; it stays scripted, fixture-only, and read-only while collecting facts, evidence, and final answers.
- Phase 13B3 tightens the guarded local-model diagnostics with safe structured error codes for non-local endpoints and local HTTP failures; see `docs/operator/phase_13b_guarded_local_model_live_loop.md`.
