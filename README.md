...
- Phase 13C adds a repeated guarded local-model live-loop variance suite in `docs/status/phase_13c_live_loop_variance_suite.md` and `docs/operator/phase_13c_live_loop_variance_suite.md`; it repeats the three hard scenarios with `third_model` and requires explicit operator `--allow-model-calls`.
- Phase 13C final evidence shows 9/9 repeated trials succeeded with stable routes and matched URLs; ticket and approval needed bounded repair, and the run stayed fixture-backed without real browser or Playwright execution.
- Phase 13D prepares a guarded Playwright replay suite for the successful Phase 13C live-loop traces; Phase 13D1 fixes the variance-suite handoff so per-trial trace files persist at the recorded relative paths, Phase 13D2 fixes the replay trace/action handoff and sanitized pre-browser diagnostics, Phase 13D3 fixes the generated operator-config shape, Phase 13D4 fixes click/navigation semantics and summary aggregation, Phase 13D5 fixes manifest-backed route mapping and visible anchors, and the final guarded Playwright replay succeeded on all three hard scenarios with `real_browser_execution: true` and `real_network_traffic: false`.
- Phase 13E adds a read-only stateful workflow foundation in `docs/status/phase_13e_readonly_stateful_workflows.md`; Phase 13E2a hardens the companion stateful local-planner packet prompt/schema and evaluator diagnostics in `docs/status/phase_13e_stateful_local_planner_packet.md`; Phase 13E2b aligns the confidence enum and truncated-output diagnostics; Phase 13E2c tightens the approval required-fact prompt and missing-key diagnostics after the follow-up `third_model` E2 run reached 4/5 accepted workflows and failed only on the approval required-fact omission; the final accepted stateful read-only planner run succeeded 5/5 with no browser or Playwright execution; Phase 13E3 materializes accepted stateful read-only planner outputs into workflow state, trace, and workflow-summary artifacts in `docs/status/phase_13e_stateful_planner_materialization.md`; Phase 13E4 adds the repeated stateful read-only planner variance suite in `docs/status/phase_13e_stateful_planner_variance.md` and `docs/operator/phase_13e_stateful_planner_variance.md`; Phase 13E4c aligns the variance prompts, fixture-workflow comparisons, and click-target diagnostics after a replay that was schema-valid but workflow-failing; Phase 13E4d narrows the remaining alignment gap by case-folding fixture-span comparison, preserving model-output citation ids in diagnostics, and explicitly reopening the hardboard before Ticket 8; Phase 13E4e tightens the remaining fact anchoring by copying the exact workspace policy marker and Ticket 8 requester tier without relaxing the evaluator; Phase 13E4f adds safe `missing_final_answer_text` diagnostics and an explicit `ticket_8_marker` anchor for the decoy phrase; Phase 13E4g fixes final-answer validation for prose containing allowed local fixture URLs; Phase 13E4h anchors policy-ticket `ticket_id` to the exact visible `Ticket 1` label after the captured variance rerun reached 12/15 workflow success with 15/15 schema validation; Phase 13E4i anchors `policy_anchor` to the exact visible `Workspace Policy` title instead of the policy URL; the final operator rerun after `a59b14f` completed the full repeated variance flow at `15/15` evaluator success and `15/15` materializer success; all stay scripted, fixture-only, and read-only while collecting facts, evidence, and final answers.
- Phase 14 adds an optional post-completion multi-model benchmark layer in `docs/status/phase_14_multimodel_benchmark.md`; the first real operator-run benchmark result shows `third_model` at `15/15` workflow success and `second_model` as a weaker baseline at `0/15` workflow success, while the evaluator remains offline and fixture-only after capture. Phase 14C also registers optional future benchmark candidates `fourth_model` and `fifth_model`; they are local-only GGUF artifacts, not part of the original TZ completion scope, and no benchmark result is claimed for them yet. Phase 14E freezes the raw benchmark contract to one shared prompt/schema/evaluator across enabled aliases, adds a broader model-neutral fixture scenario catalog, adds a sequential multi-model runner, hardens that runner with structured timeout/connection handling and benchmark-port preflight, applies a shared `max_tokens: 4096` budget equally across enabled models, and keeps `fifth_model` available but disabled in routine default runs because of observed slowness. The clean runner-owned Phase 14E rerun is now recorded: `fourth_model` led `third_model` on the default frozen raw benchmark at `6/8` versus `5/8` workflow success, with no model-specific prompt tuning; that is a bounded fixture-only benchmark result, not a universal model-quality claim. Phase 14F now has a completed five-model final-presentation run: `fourth_model` ranked first on the specific 11-scenario frozen raw fixture-only benchmark at `5/11` workflow success, tied with `third_model` on completed workflows but ahead on validation acceptance; `fifth_model` reached `3/11`, and both lower baselines `first_model` and `second_model` reached `1/11`. Important framing: that Phase 14F line is a frozen raw one-shot workflow-JSON planner benchmark, not a high-confidence long-horizon interactive browser-agent benchmark; a PASS/FAIL can mix schema validity, action choice, workflow materialization, and final-answer quality, and even semantically reasonable answers can fail if the workflow proposes an invalid action such as clicking non-clickable text. Phase 15A starts a separate fixture-only stepwise observation-action article benchmark to measure that interactive gap without claiming production readiness.
- Phase 14 adds an optional post-completion multi-model benchmark layer in `docs/status/phase_14_multimodel_benchmark.md`; the first real operator-run benchmark result shows `third_model` at `15/15` workflow success and `second_model` as a weaker baseline at `0/15` workflow success, while the evaluator remains offline and fixture-only after capture. Phase 14C also registers optional future benchmark candidates `fourth_model` and `fifth_model`; they are local-only GGUF artifacts, not part of the original TZ completion scope, and no benchmark result is claimed for them yet. Phase 14E freezes the raw benchmark contract to one shared prompt/schema/evaluator across enabled aliases, adds a broader model-neutral fixture scenario catalog, adds a sequential multi-model runner, hardens that runner with structured timeout/connection handling and benchmark-port preflight, applies a shared `max_tokens: 4096` budget equally across enabled models, and keeps `fifth_model` available but disabled in routine default runs because of observed slowness. The clean runner-owned Phase 14E rerun is now recorded: `fourth_model` led `third_model` on the default frozen raw benchmark at `6/8` versus `5/8` workflow success, with no model-specific prompt tuning; that is a bounded fixture-only benchmark result, not a universal model-quality claim. Phase 14F now has a completed five-model final-presentation run: `fourth_model` ranked first on the specific 11-scenario frozen raw fixture-only benchmark at `5/11` workflow success, tied with `third_model` on completed workflows but ahead on validation acceptance; `fifth_model` reached `3/11`, and both lower baselines `first_model` and `second_model` reached `1/11`. Important framing: that Phase 14F line is a frozen raw one-shot workflow-JSON planner benchmark, not a high-confidence long-horizon interactive browser-agent benchmark; a PASS/FAIL can mix schema validity, action choice, workflow materialization, and final-answer quality, and even semantically reasonable answers can fail if the workflow proposes an invalid action such as clicking non-clickable text.
- Phase 15 starts a separate fixture-only stepwise observation-action article benchmark in `docs/status/phase_15_stepwise_article_benchmark.md`; Phase 15A adds the deterministic offline foundation, and Phase 15B adds a guarded local-model adapter that requests exactly one action JSON per step, keeps `browser_click` out of the default article action set, and requires explicit `--allow-model-execution` before any local model call.
- The project-wide closure report is `docs/status/final_project_completion_report.md`; it maps the implemented architecture and final evidence back to the original TZ, including the conservative resource formula and the final `third_model` repeated stateful planner result.
- Phase 13B3 tightens the guarded local-model diagnostics with safe structured error codes for non-local endpoints and local HTTP failures; see `docs/operator/phase_13b_guarded_local_model_live_loop.md`.
...
- Phase 11B prepares a dual-scenario local planner operator packet for `browser_ticket_triage_review` and `browser_approval_form_review`; it adds two compact prompt files and stays offline.
...
- Phase 10.10b documents guarded fixture-backed replay evidence in `docs/status/model_plan_guarded_fixture_replay_evidence.md`; it confirms the runner replays through fixture-backed actions, not real Playwright.
