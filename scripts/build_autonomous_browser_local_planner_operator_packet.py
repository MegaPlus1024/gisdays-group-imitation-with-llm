...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build an offline local planner operator packet for future manual local planner runs."
    )
    ...
    try:
        ...
        summary = build_autonomous_browser_local_planner_operator_packet(config, repo_root=PROJECT_ROOT)
...
def _load_config(path: Path) -> dict[str, Any]:
    ...
    if str(payload.get("schema_version", "")) != PACKET_CONFIG_SCHEMA_VERSION:
        raise ValueError("config schema_version must match autonomous_browser_local_planner_operator_packet_config_v1.")
