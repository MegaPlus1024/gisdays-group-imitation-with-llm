...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build the Phase 11 local planner packet.")
    parser.add_argument("--config", required=True)
    args = parser.parse_args(argv)

    try:
        config = _load_config(Path(args.config))
        summary = build_autonomous_browser_phase11_local_planner_packet(config, repo_root=PROJECT_ROOT)
...
def _load_config(path: Path) -> dict[str, Any]:
    ...
    if str(payload.get("schema_version", "")) != PACKET_CONFIG_SCHEMA_VERSION:
        raise ValueError(
            "config schema_version must match autonomous_browser_phase11_local_planner_packet_config_v1."
        )
