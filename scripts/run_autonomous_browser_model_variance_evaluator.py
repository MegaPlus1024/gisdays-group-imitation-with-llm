...
from src.agent.autonomous_browser_model_variance_evaluator import (
    ...
    write_autonomous_browser_model_variance_evaluator_summary,
)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the offline repeated hard-trials variance evaluator.")
    ...
    try:
        ...
        summary = run_autonomous_browser_model_variance_evaluator(
            config,
        ...
        if output_dir:
            write_autonomous_browser_model_variance_evaluator_summary(summary, _resolve_repo_path(output_dir))
...
def _load_config(path: Path) -> dict[str, Any]:
    ...
    if str(payload.get("schema_version", "")) != CONFIG_SCHEMA_VERSION:
        raise ValueError("config schema_version must match autonomous_browser_model_variance_evaluator_config_v1.")
