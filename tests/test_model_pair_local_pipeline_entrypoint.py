...
def test_config_build_helper_accepts_compact_repair_execute_v2_config(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls = _install_fake_existing_pipeline_module(monkeypatch)
    local_config = json.loads(
        Path("configs/local_pipeline/single_trial_local_pipeline.dual_endpoint.compact_repair_execute_v2.example.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    assert config.payload["mode"] == "local"
    assert config.payload["run_id"] == "phase_8_23_office_extension_retry"
    assert config.payload["out_dir"] == "artifacts/single_trial_runs/phase_8_23_office_extension_retry/pipeline"
    assert config.payload["execute_actions"] is True
    assert config.payload["action_parameter_repair"]["office_default_output_dir"].endswith(
    ...
    assert config.payload["office_real_document_artifact_root"].endswith(
        "phase_8_23_office_extension_retry/pipeline/workspace"
    )
    assert [name for name, _ in calls] == ["config"]
