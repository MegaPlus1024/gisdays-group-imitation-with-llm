...
- A bounded concurrent stress smoke now exists under `experiments/multi_agent/orchestrator_executor/bounded_stress_candidate_pairs_v1` with explicit `strict_cpu` and `gpu_full_offload` profiles. It preserved telemetry and failures, but all attempted heavy action-execution batches failed and no stable concurrency was observed.
...
| group of agents | `MultiAgentOrchestratorSmoke`, multi-agent scenario configs, orchestrator/executor artifacts, `docs/ai/bounded_stress_candidate_pairs_v1.md` | partially complete / MVP implemented | Sequential fake-mode and local pair-matrix evidence exist; bounded concurrency was attempted but every batch failed. | Fix the heavy action/workspace failure path and rerun bounded stress. |
| orchestrator/executor pair | `src/agent/orchestrator_executor_pipeline.py`, `scripts/run_orchestrator_executor_group.py`, repeated local artifacts, pair matrices, `docs/ai/heavy_scenario_error_analysis_v1.md`, `docs/ai/orchestrator_executor_runtime_capacity_v1.md`, `docs/ai/gpu_smoke_second_to_second_heavy_v1.md`, `docs/ai/bounded_stress_candidate_pairs_v1.md` | partially complete | Pair comparison covers two group scenarios, one short runtime/resource probe, one GPU smoke, and one failed bounded stress matrix. | Add scenario diversity and rerun bounded stress after action/workspace fix. |
...
| GPU runtime | GPU audit detected `NVIDIA RTX PRO 4000 Blackwell`; `llama-server --help` exposes GPU flags; wrapper flags, N=1 GPU smoke, and bounded GPU stress attempt exist | partially complete | GPU runtime configured: yes; GPU runtime measured: yes, but bounded stress failed before completed group runs. Meaningful speedup/capacity is not established. | Fix the heavy action/workspace failure path and rerun bounded stress with explicit GPU monitoring. |
| multi-agent capacity | Formula estimate exists; runtime probe measured pair RSS/CPU and produced RAM-based capacity estimates; bounded stress attempted levels 1 and 2 | estimated only / partially measured | Concurrent stress telemetry exists, but no stable concurrency was observed because all batches failed. | Rerun controlled concurrent stress after workspace/action failure is fixed. |
...
- Group of agents: partially complete; sequential fake-mode and local repeated/matrix evidence now exist, including a four-agent heavy scenario and a failed bounded concurrency smoke.
...
- GPU runtime: hardware detected, wrapper flags implemented, one short GPU smoke completed, and bounded GPU stress was attempted. Stress readiness is still preliminary because all bounded stress batches failed.
- Measured multi-agent capacity: partially measured for short sequential pair telemetry; bounded concurrency telemetry exists, but stable concurrent capacity is still estimated only / not observed.
